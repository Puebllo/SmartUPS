 ...
 // return true;
   ...
   //look for place for session
    ...
    if (ws.ipAddress.equals("-1")){
      webSession wsNew;
      Serial.print("Found place for session with ip: " ); Serial.println(sessionIp.toString());
  ...
  Serial.print("reached max sessions = "); Serial.println(sizeof(sessions));
    return false; 
...
void validateSessions(){
    ...
    if (!ws.ipAddress.equals("-1")){
         unsigned long maxTime = ws.loginTime + SESSION_EXPIRE_TIME;
      if (millis()>maxTime){
              Serial.print("Releasing session with ip: " ); Serial.println(ws.ipAddress);
...
void clearSessions(){
  ...
  sessions[i] = {"-1",0};
           ...
           for (int i = 0; i < 3; i++){
              webSession ws = sessions[i];
  Serial.print("Session ");   Serial.print(i);   Serial.print(". - IP: ");   Serial.println(ws.ipAddress);
...
String getUPSData(IPAddress sessionIp)
{
  ...
  return String(voltage) + ";" + String(voltPrec) + ";" + String(stateOfCharge) + ";" + String(counter) + ";" + getUptime() + ";" + getRemainingSessionTime(sessionIp);
}
...
void startServer() {
 ...
 server.on("/login", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send(LittleFS, "/loginPage.html", "text/html", false, processor);
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        if (!wifiConnected){
            Serial.println("Knock ap");
                  request->send(LittleFS, "/ap_index.html", "text/html", false, processor);
        }else{
...
request->send(401);
    ...
    server.on("/bootstrap.min.js", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send(LittleFS, "/bootstrap.min.js", "application/javascript");
    });
    ...
    server.on("/refreshData", HTTP_GET, [](AsyncWebServerRequest *request) {
      ...
      request->send_P(200, "text/plain", getUPSData(request->client()->remoteIP()).c_str());
            }else{
request->send(401);
            }
    ...
    server.on("/initData", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send_P(200, "text/plain", getInitData().c_str());
        ...
        server.on("/getPrecCalibData", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send_P(200, "text/plain", getPrecCalibData().c_str());
    });

    //Calibration posts
        server.on("/setAdcPrecFactor", HTTP_POST, [](AsyncWebServerRequest *request) {
...
request->redirect("/calib");
        ...
        server.on("/setVDPrecFactor", HTTP_POST, [](AsyncWebServerRequest *request) {
...
request->redirect("/calib");
            ...
            server.on("/setPowerVoltage", HTTP_POST, [](AsyncWebServerRequest *request) {
...
if(reqLogin.equals(webLogin) && reqPwd.equals(webPassword)){
  boolean sessionPersisted = saveUserSession(millis(),request->client()->remoteIP());
  if (sessionPersisted){
...
AsyncWebServerResponse *response = request->beginResponse(LittleFS, "/loginPage.html", "text/html", false, processor);
